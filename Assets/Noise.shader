Shader "Custom/Noise"
{
    ...
    SubShader
    {
        ...
        float ApplyNoise(float3 p) {
            ...
            float displacement = _Size * cnoise(_Frequency * p + _Time.y * 0.3);
            return displacement;
        }
        ...
        {
            float noise = sin(_Frequency * IN.worldPos.y + _Time.y );
            float mapped = noise * 0.5 + 0.5;
            float3 tint = lerp(float3(1.0, 0.85, 0.9), float3(0.85, 0.9, 1.0), mapped);
            float alpha = _Color.a * (0.4 + 0.6 * mapped);

            o.Albedo = _Color.rgb * tint;
            o.Alpha = _Color.a * (0.4 + 0.6 * mapped); // allows fade based on alpha AND animation
            o.Emission = (1.0 - alpha) * float3(1.0, 1.0, 1.0) * 0.7;
